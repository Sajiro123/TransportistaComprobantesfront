...
> El registro **no** inicia sesión automáticamente. Al completar el paso 3 solo se devuelve el `usuarioUuid`; luego el usuario debe autenticarse en `POST /api_iam/auth/login`.
...
| `REG_002` | 400 | Usuario/correo ya registrado | Sugerir ir a login |
| `REG_003` | 400 | No se pudo completar el registro | Reintentar / soporte |
...
function mensajeDeError(error) {
  ...
  switch (err?.code) {
    ...
    case "REG_002": return "El correo ya está registrado. Intenta iniciar sesión.";
...
// alta.usuarioUuid -> redirigir a login
```
...
| `fn_registrar_usuario(...)` | Alta del usuario. **El DBA debe completar el INSERT real y la validación de unicidad** según el modelo de `n_iam.usuario` |

Resultados posibles de `fn_registrar_usuario` mapeados por el backend:

- `REGISTRADO` → éxito, devuelve `usuarioUuid`.
- `YA_EXISTE` → el backend responde `REG_002` (correo/usuario ya registrado).
- Cualquier otro → `REG_003` (no se pudo completar el registro).
